...
import { generatePodcast, Dependencies } from '../src/cli.js';
...
describe('CLI - generatePodcast', () => {
  ...
  beforeEach(() => {
    ...
    mockMonologueEngine = {
      generateMonologue: vi.fn().mockResolvedValue([
        { timestamp: '00:00', text: 'Test content', emotion: 'neutral', duration: 30 }
      ])
    };
    ...
    mockDeps = {
      ...
      MonologueEngine: vi.fn().mockImplementation(() => mockMonologueEngine) as any,
      ScriptFormatter: vi.fn().mockImplementation(() => mockScriptFormatter) as any,
      AudioSynthesizer: vi.fn().mockImplementation(() => mockAudioSynthesizer) as any,
  ...
  describe('Successful podcast generation', () => {
    test('shouldGeneratePodcastWithValidInputs', async () => {
      ...
      expect(mockDeps.validateTopic).toHaveBeenCalledWith('artificial intelligence');
    ...
    test('shouldCreateOutputDirectoryAndInstantiateEngines', async () => {
      ...
      expect(mockDeps.fs.ensureDir).toHaveBeenCalledWith('/resolved/output/path');
    ...
    test('shouldExecuteGenerationStepsInCorrectOrder', async () => {
      ...
      expect(mockMonologueEngine.generateMonologue).toHaveBeenCalledWith('machine learning', 5);
    ...
    test('shouldShowProgressAndSuccessMessages', async () => {
      ...
      expect(mockDeps.showProgress).toHaveBeenCalledWith("Generating podcast on: 'quantum computing'");
      expect(mockDeps.showSuccess).toHaveBeenCalledWith('Podcast generated successfully!');
      
      // And: Should show file outputs
      expect(mockDeps.showFileOutput).toHaveBeenCalledWith('script', '/resolved/output/path/test-topic_2025-07-12.json');
      expect(mockDeps.showFileOutput).toHaveBeenCalledWith('audio', '/resolved/output/path/test-topic_2025-07-12.mp3');
    });
  });
  ...
  describe('Input validation and processing', () => {
    test('shouldParseDurationStringToNumber', async () => {
      ...
      expect(mockMonologueEngine.generateMonologue).toHaveBeenCalledWith('test', 10);
    });
    ...
    test('shouldHandleSpecialCharactersInTopic', async () => {
      ...
      expect(mockDeps.validateTopic).toHaveBeenCalledWith('AI & Machine Learning: The Future (2025)!');
      expect(mockMonologueEngine.generateMonologue).toHaveBeenCalledWith('AI & Machine Learning: The Future (2025)!', 5);
    });
  });
  ...
  describe('Error handling', () => {
    test('shouldThrowWhenValidationFails', async () => {
      ...
      await expect(generatePodcast(topic, options, mockDeps))
        .rejects
        .toThrow('Topic validation failed');
    ...
    test('shouldThrowWhenDurationValidationFails', async () => {
      ...
      await expect(generatePodcast(topic, options, mockDeps))
        .rejects
        .toThrow('Duration must be 5 or 10 minutes');
    });
    ...
    test('shouldThrowWhenFileSystemOperationFails', async () => {
      ...
      const topic = 'test';
      const options: CliOptions = { duration: '5', output: './output' };
      (mockDeps.fs.ensureDir as any).mockRejectedValue(new Error('Permission denied'));
      ...
      expect(mockDeps.showProgress).toHaveBeenCalledWith("Generating podcast on: 'test'");
    });
    ...
    test('shouldThrowWhenMonologueGenerationFails', async () => {
      ...
      const topic = 'test';
      const options: CliOptions = { duration: '5', output: './output' };
      mockMonologueEngine.generateMonologue.mockRejectedValue(new Error('API failure'));
      ...
      expect(mockDeps.showStep).toHaveBeenCalledWith(1, 4, 'Analyzing topic...');
      // But: Should not have proceeded to step 2
      expect(mockDeps.showStep).not.toHaveBeenCalledWith(2, 4, 'Creating narrative content...');
    });
    ...
    test('shouldThrowWhenScriptFormattingFails', async () => {
      ...
      const topic = 'test';
      const options: CliOptions = { duration: '5', output: './output' };
      mockScriptFormatter.formatScript.mockRejectedValue(new Error('Formatting error'));
      ...
      expect(mockDeps.showStep).toHaveBeenCalledWith(2, 4, 'Creating narrative content...');
    });
    ...
    test('shouldThrowWhenAudioSynthesisFails', async () => {
      ...
      const topic = 'test';
      const options: CliOptions = { duration: '5', output: './output' };
      mockAudioSynthesizer.synthesizeAudio.mockRejectedValue(new Error('Audio generation failed'));

      // When/Then: Should throw audio synthesis error
      await expect(generatePodcast(topic, options, mockDeps))
        .rejects
        .toThrow('Audio generation failed');

      // And: Should have completed step 3 but failed at step 4
      expect(mockDeps.showStep).toHaveBeenCalledWith(4, 4, 'Synthesizing voice...');
    });
  });
  ...
  describe('Edge cases', () => {
    test('shouldHandleEmptySegmentsFromMonologueEngine', async () => {
      ...
      expect(mockAudioSynthesizer.synthesizeAudio).toHaveBeenCalledWith([], expect.any(String));
    });
    ...
    test('shouldHandleComplexFilenames', async () => {
      ...
      const options: CliOptions = { duration: '5', output: './output' };
      mockScriptFormatter.generateFilename.mockReturnValue('complex-topic-with-symbols_2025-07-12');
