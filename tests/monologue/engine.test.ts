...
vi.mock('../../src/monologue/prompts.js', () => ({
  ...
  createMonologuePrompt: vi
    .fn()
    .mockReturnValue('Generated prompt for testing'),
  NARRATIVE_PHASES: {
    introduction: {
      description: 'Test intro',
      instructions: 'Test intro instructions',
      targetPercentage: 15,
    },
    exploration: {
      description: 'Test exploration',
      instructions: 'Test exploration instructions',
      targetPercentage: 70,
    },
    conclusion: {
      description: 'Test conclusion',
      instructions: 'Test conclusion instructions',
      targetPercentage: 15,
    },
  },
}));
...
describe('MonologueEngine', () => {
  ...
  describe('Constructor', () => {
    ...
    test('shouldAcceptInjectedLLMClient', () => {
      // Given: Mock LLM client
      const customMockClient: LLMService = {
        generateContent: vi.fn(),
        isHealthy: vi.fn(),
      };
  ...
  describe('generateMonologue - Phase Distribution', () => {
    beforeEach(() => {
      ...
      let callCount = 0;
      (mockLLMClient.generateContent as any).mockImplementation(
  ...
  describe('Utility Methods', () => {
    ...
    describe('cleanText', () => {
      ...
      test('shouldRemoveMultipleEmotionMarkers', () => {
        ...
        const content =
          'Start [thoughtful] middle [curious] end [reflective] done.';
    ...
    describe('estimateDuration', () => {
      ...
      test('shouldScaleWithTextLength', () => {
        ...
        const longText =
          'This is a much longer piece of text that should take significantly more time to read aloud compared to the shorter version above.';
  ...
  describe('Error Handling', () => {
    test('shouldWrapGenerationErrorsInPodcastGenerationError', async () => {
      ...
      (mockLLMClient.generateContent as any).mockRejectedValue(
        new LLMNetworkError('Network error')
      );

      // When/Then: Should wrap error appropriately
      await expect(engine.generateMonologue('test topic', 5)).rejects.toThrow(
    ...
    test('shouldHandleAuthenticationErrors', async () => {
      ...
      (mockLLMClient.generateContent as any).mockRejectedValue(
        new LLMAuthenticationError('Authentication failed')
      );
    ...
    test('shouldHandleNetworkErrors', async () => {
      ...
      (mockLLMClient.generateContent as any).mockRejectedValue(
        new LLMNetworkError('Connection failed')
      );
    ...
    test('shouldHandleGenericLLMErrors', async () => {
      ...
      (mockLLMClient.generateContent as any).mockRejectedValue(
        new LLMError('Generic LLM error', 'UNKNOWN', false)
      );
    ...
    test('shouldCallLLMClientWithCorrectParameters', async () => {
      ...
      requestCapture.forEach((request) => {
        expect(request.systemPrompt).toContain(
          'Test personality for engaging podcast narration'
        );
        expect(request.systemPrompt).toContain(
          'Test instructions for formatting responses'
        );
